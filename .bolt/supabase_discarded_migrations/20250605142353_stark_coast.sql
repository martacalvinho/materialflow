/*
  # Enhance Schema with Enums and Additional Fields
  
  1. New Tables
    - studios: Organization/company management
    - uploaded_files: PDF file tracking
  
  2. Enums and Additional Fields
    - Add type safety with enums
...
-- Convert role column to enum (two-step process)
ALTER TABLE users 
...
ALTER TABLE users 
  ALTER COLUMN role TYPE user_role_enum USING 
    CASE role
      ...
      WHEN 'studio_owner' THEN 'studio_owner'::user_role_enum
...
-- Convert project type and status columns to enums
ALTER TABLE projects 
...
ALTER TABLE materials
  ADD COLUMN IF NOT EXISTS unit_type material_unit_enum DEFAULT 'square_feet'::material_unit_enum,
...
-- Convert material category to enum
ALTER TABLE materials 
...
CREATE POLICY "Studio owners can manage their studio" ON studios
  ...
  USING (id IN (
    ...
    WHERE id = auth.uid() AND role = 'studio_owner'::user_role_enum
  ))
  WITH CHECK (id IN (
    SELECT studio_id FROM users 
    WHERE id = auth.uid() AND role = 'studio_owner'::user_role_enum
